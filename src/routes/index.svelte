...
<div class="text-center py-40">
	...
	<form on:submit|preventDefault={handleSubmit} class="w-full sm:w-80 mt-4 mx-auto space-y-2">
		<input type="text" bind:value={url} class="border border-gray-300 p-3 w-full focus:outline-none focus:border-blue-500 ring-2 ring-transparent focus:ring-blue-500" placeholder="URL, ej: dsoto.dev">
		{#if error}
...
<div class="mb-12">
	...
	<div class="grid sm:grid-cols-2 md:grid-cols-4 2xl:grid-cols-5 gap-10">
		...
		<div >
			<a href={blog.url} class="group " target="_blank" rel="noopener nofollower">
				...
				<div>
					...
					<p class="transition text-gray-500 group-hover:text-gray-700">
						{blog.description}
					</p>
				</div>
			</a>
